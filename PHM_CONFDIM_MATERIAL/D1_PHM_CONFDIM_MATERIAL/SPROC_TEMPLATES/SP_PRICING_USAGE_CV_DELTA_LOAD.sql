    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_PRICING_USAGE_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for PRICING_USAGE_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.PRICING_USAGE_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.PRICING_USAGE_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.PRICING_USAGE_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.YYPRIU_YTMM_PRIU_T = SOURCE.YYPRIU_YTMM_PRIU_T,
        TARGET.YYDESC_YTMM_PRIU_T = SOURCE.YYDESC_YTMM_PRIU_T,
    ...
    WHEN NOT MATCHED THEN
      INSERT VALUES
      (
        SOURCE.YYPRIU_YTMM_PRIU_T,
        SOURCE.YYDESC_YTMM_PRIU_T,
        SOURCE.ROW_ADD_STP,
