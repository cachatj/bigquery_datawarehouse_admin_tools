    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_HAZMAT_STORAGE_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for HAZMAT_STORAGE_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.HAZMAT_STORAGE_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.HAZMAT_STORAGE_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.HAZMAT_STORAGE_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.YYHAZSC_YTMM_HAZMAT_TEXT = SOURCE.YYHAZSC_YTMM_HAZMAT_TEXT,
        TARGET.YYDESC_YTMM_HAZMAT_TEXT = SOURCE.YYDESC_YTMM_HAZMAT_TEXT,
    ...
    WHEN NOT MATCHED THEN
      INSERT VALUES
      (
        SOURCE.YYHAZSC_YTMM_HAZMAT_TEXT,
        SOURCE.YYDESC_YTMM_HAZMAT_TEXT,
        SOURCE.ROW_ADD_STP,
