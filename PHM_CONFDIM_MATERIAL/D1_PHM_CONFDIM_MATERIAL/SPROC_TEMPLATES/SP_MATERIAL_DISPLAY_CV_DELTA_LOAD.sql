    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_MATERIAL_DISPLAY_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for MATERIAL_DISPLAY_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.MATERIAL_DISPLAY_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.MATERIAL_DISPLAY_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.MATERIAL_DISPLAY_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.MATNR_MAST = SOURCE.MATNR_MAST,
        TARGET.MAKTX_MAKT = SOURCE.MAKTX_MAKT,
        TARGET.IDNRK_STPO = SOURCE.IDNRK_STPO,
        TARGET.MAKTX_MAKT_COMP = SOURCE.MAKTX_MAKT_COMP,
        TARGET.MENGE_STPO = SOURCE.MENGE_STPO,
        TARGET.MEINS_STPO = SOURCE.MEINS_STPO,
        TARGET.MSEHL_T006A = SOURCE.MSEHL_T006A,
