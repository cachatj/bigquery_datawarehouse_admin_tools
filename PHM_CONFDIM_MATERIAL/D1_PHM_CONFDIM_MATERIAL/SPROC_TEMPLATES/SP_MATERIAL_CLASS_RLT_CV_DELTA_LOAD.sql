    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_MATERIAL_CLASS_RLT_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for MATERIAL_CLASS_RLT_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.MATERIAL_CLASS_RLT_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.MATERIAL_CLASS_RLT_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.MATERIAL_CLASS_RLT_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        ...
        TARGET.CALC_CLASS_TYPE_ID = SOURCE.CALC_CLASS_TYPE_ID,
        TARGET.CALC_CLASS_CRCTRSTC_VALUE_ID = SOURCE.CALC_CLASS_CRCTRSTC_VALUE_ID,
    ...
    WHEN NOT MATCHED THEN
      ...
      (
        SOURCE.MATNR_MARA,
        SOURCE.CALC_CLASS_TYPE_ID,
        SOURCE.CALC_CLASS_CRCTRSTC_VALUE_ID,
        SOURCE.ROW_ADD_STP,
