    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_MTRL_GRP_PACKAGING_MATERIALS_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for MTRL_GRP_PACKAGING_MATERIALS_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.MTRL_GRP_PACKAGING_MATERIALS_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.MTRL_GRP_PACKAGING_MATERIALS_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.MTRL_GRP_PACKAGING_MATERIALS_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.MAGRV_TVEGRT = SOURCE.MAGRV_TVEGRT,
        TARGET.BEZEI_TVEGRT = SOURCE.BEZEI_TVEGRT,
