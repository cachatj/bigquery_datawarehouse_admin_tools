    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_PACKAGING_INDICATOR_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for PACKAGING_INDICATOR_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.PACKAGING_INDICATOR_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.PACKAGING_INDICATOR_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.PACKAGING_INDICATOR_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.YYPCKG_YTMM_PACK_TEXT = SOURCE.YYPCKG_YTMM_PACK_TEXT,
        TARGET.YYDESC_YTMM_PACK_TEXT = SOURCE.YYDESC_YTMM_PACK_TEXT,
    ...
    WHEN NOT MATCHED THEN
      INSERT VALUES
      (
        SOURCE.YYPCKG_YTMM_PACK_TEXT,
        SOURCE.YYDESC_YTMM_PACK_TEXT,
        SOURCE.ROW_ADD_STP,
