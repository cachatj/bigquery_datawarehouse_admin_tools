    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_HAMACHER_FINEST_CLASS_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for HAMACHER_FINEST_CLASS_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.HAMACHER_FINEST_CLASS_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.HAMACHER_FINEST_CLASS_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.HAMACHER_FINEST_CLASS_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.YYFINEST_YTMM_FINEST_TEXT = SOURCE.YYFINEST_YTMM_FINEST_TEXT,
        TARGET.YYDESC_YTMM_FINEST_TEXT = SOURCE.YYDESC_YTMM_FINEST_TEXT,
    ...
    WHEN NOT MATCHED THEN
      INSERT VALUES
      (
        SOURCE.YYFINEST_YTMM_FINEST_TEXT,
        SOURCE.YYDESC_YTMM_FINEST_TEXT,
        SOURCE.ROW_ADD_STP,
