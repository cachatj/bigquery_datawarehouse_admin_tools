    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_FDB_NDC_PRICE_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for FDB_NDC_PRICE_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.FDB_NDC_PRICE_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.FDB_NDC_PRICE_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.FDB_NDC_PRICE_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        ...
        TARGET.PRICE_TYPE_DESCRIPTION = SOURCE.PRICE_TYPE_DESCRIPTION,
        TARGET.PRICE = SOURCE.PRICE,
        TARGET.PRICE_EFFECTIVE_DT = SOURCE.PRICE_EFFECTIVE_DT,
    ...
    WHEN NOT MATCHED THEN
      ...
      (
        SOURCE.NDC,
        SOURCE.PRICE_TYPE,
        SOURCE.PRICE_TYPE_DESCRIPTION,
        SOURCE.PRICE,
        SOURCE.PRICE_EFFECTIVE_DT,
        SOURCE.ROW_ADD_STP,
