    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_REMS_PROGRAM_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for REMS_PROGRAM_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.REMS_PROGRAM_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.REMS_PROGRAM_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.REMS_PROGRAM_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.YYREMSP_YTMM_REMS_PROG_T = SOURCE.YYREMSP_YTMM_REMS_PROG_T,
        TARGET.YYDESC_YTMM_REMS_PROG_T = SOURCE.YYDESC_YTMM_REMS_PROG_T,
    ...
    WHEN NOT MATCHED THEN
      INSERT VALUES
      (
        SOURCE.YYREMSP_YTMM_REMS_PROG_T,
        SOURCE.YYDESC_YTMM_REMS_PROG_T,
        SOURCE.ROW_ADD_STP,
