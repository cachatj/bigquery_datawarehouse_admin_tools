    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.SP_AHFS_CODE_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for AHFS_CODE_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_MATERIAL.AHFS_CODE_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.AHFS_CODE_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_MATERIAL.AHFS_CODE_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.YYAHFS_YTMM_AHFS_TEXT = SOURCE.YYAHFS_YTMM_AHFS_TEXT,
        TARGET.YYDESC_YTMM_AHFS_TEXT = SOURCE.YYDESC_YTMM_AHFS_TEXT,
    ...
    WHEN NOT MATCHED THEN
      INSERT VALUES
      (
        SOURCE.YYAHFS_YTMM_AHFS_TEXT,
        SOURCE.YYDESC_YTMM_AHFS_TEXT,
        SOURCE.ROW_ADD_STP,
