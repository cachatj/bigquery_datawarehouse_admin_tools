...
# DAG Definition
with DAG(
    'pharma_pdwmtrl_material_sp_expiration_date_indicator_cv_manual',
    default_args=default_args,
    description=f'Process for updating pharma_pdwmtrl_material_sp_expiration_date_indicator_cv_manual',
    schedule_interval=schedule_interval,
    start_date=pendulum.datetime(2023, 9, 8, tz='US/Eastern'),
    catchup=False,
    tags=['pdw', 'material','insert_sql'],  # Added the dag_domain as a tag
    access_control= {"pdna" : ('can_read', 'can_edit')},
    concurrency=50,  # Add this line to set the concurrency
    max_active_runs = 1,
) as dag:

    execute_sp_expiration_date_indicator_cv_manual = BigQueryInsertJobOperator(
        task_id='execute_sp_expiration_date_indicator_cv_manual',
        configuration={
            'query': {
                'query': f"CALL `{project_id}`.D1_PHM_CONFDIM_MATERIAL.SP_EXPIRATION_DATE_INDICATOR_CV_MANUAL();",
