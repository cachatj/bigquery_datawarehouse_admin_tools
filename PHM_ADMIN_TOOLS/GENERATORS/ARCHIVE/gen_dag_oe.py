...
if airflow_env == "dev":
    service_account = 'sa-phm-pd-oe-conv@edc-p-oe-np-cah.iam.gserviceaccount.com'
    compute_project_id = 'edc-p-oe-np-cah'

elif airflow_env == "stg":
    service_account = 'sa-phm-pd-oe-conv@edc-p-oe-stg-pr-cah.iam.gserviceaccount.com'
    compute_project_id = 'edc-p-oe-stg-pr-cah'

elif airflow_env == "prd":
    service_account = 'sa-phm-pd-oe-conv@edc-p-oe-pr-cah.iam.gserviceaccount.com'
    compute_project_id = 'edc-p-oe-pr-cah'
...
with DAG(
    ...
    schedule_interval={ 'None' if dag_metadata['sched_interval'] is None else f"'{dag_metadata['sched_interval']}'"},
    start_date=datetime(2023, 9, 8),
    catchup=False,
    tags=['oe', 'tdr', '{dag_metadata['dag_domain']}','{dag_metadata['dag_type'].lower()}'],  # Added the dag_domain as a tag
...
if __name__ == "__main__":
    # Read metadata CSV
    csv_path = 'PHM_ADMIN_TOOLS\INPUT_FILES\dag_metada_oe.csv'
    ...
    for dag_name, group in grouped_dag_metadata:
        ...
        sched_interval = group['sched_interval'].iloc[0]
        if sched_interval == 'None' or pd.isna(sched_interval):
            sched_interval = None
