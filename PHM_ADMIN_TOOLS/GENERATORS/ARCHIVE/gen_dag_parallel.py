...
"""
    ...
    # Generate task code
    tasks = ""
    for task_group in dag_metadata['tasks']:
...
'''
            ...
            query_str = f"CALL `{project_id}`.{task_group['table_schema']}.{task_group['sproc_call']};"
            tasks += f'''
    {task_group['task_id']} = BigQueryInsertJobOperator(
        task_id='{task_group['task_id']}',
        configuration={{
            'query': {{
                'query': f"{query_str}",
                'use_legacy_sql': False
...
'''
    ...
    dependencies = ""
    for i in range(len(dag_metadata['tasks']) - 1):
        if isinstance(dag_metadata['tasks'][i], list):
            for task in dag_metadata['tasks'][i]:
                dependencies += f"    {task['task_id']} >> {dag_metadata['tasks'][i+1]['task_id']}\n"
        else:
            dependencies += f"    {dag_metadata['tasks'][i]['task_id']} >> {dag_metadata['tasks'][i+1]['task_id']}\n"
...
if __name__ == "__main__":
    # Read metadata CSV
    csv_path = 'pharma-data-warehouse-edna\PHM_ADMIN_TOOLS\INPUT_FILES\dag_metada.csv'
    ...
    for dag_name, group in grouped_dag_metadata:
        ...
        for _, row in sorted_group.iterrows():
            ...
            current_task_seq = row['Task Number Seq']
            if current_task_seq == 500:
                parallel_tasks.append(task)
            else:
                if parallel_tasks:
                    tasks.append(parallel_tasks)
                    parallel_tasks = []
                tasks.append(task)
            previous_task_seq = current_task_seq
        if parallel_tasks:
            tasks.append(parallel_tasks)
