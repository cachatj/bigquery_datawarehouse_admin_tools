...
#repo_path = 'puerto_rico_data_warehouse_looker/'
#branch_name = 'project_admin'

# Call the function
#git_operations.switch_to_branch(repo_path, branch_name)

# Initialize BigQuery client
#client = bigquery.Client(project='ednacomp-corp-np-cah')
...
SELECT DISTINCT
   ...
   -- looker_label,
    CASE
      WHEN vw_schema  LIKE '%pr_sales_cube%'  AND vw_entity_name IN('pr_transaction_type','pr_contract','pr_date','pr_date_sales','pr_market_share','pr_salesman','pr_supplier','pr_supplier_extended','pr_product_extended','pr_sales','pr_product','pr_customer','pr_customer_extended') THEN INITCAP(REPLACE(vw_column_name,"_"," "))
      ELSE looker_label
    END AS looker_label,
    ...
    CASE 
      ...
      WHEN LOWER(vw_schema) LIKE '%pr_sales_cube%' THEN 'puerto_rico_data_warehouse_looker'
...
WHERE 1=1
  ...
  AND vw_schema LIKE '%pr_sales_cube%'
