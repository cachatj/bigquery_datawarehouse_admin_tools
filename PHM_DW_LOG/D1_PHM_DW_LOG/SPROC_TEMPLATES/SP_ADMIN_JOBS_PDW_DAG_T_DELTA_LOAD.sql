    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_DW_LOG.SP_ADMIN_JOBS_PDW_DAG_T_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for ADMIN_JOBS_PDW_DAG_T
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_DW_LOG.ADMIN_JOBS_PDW_DAG_T_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_JOBS_PDW_DAG_T`);


    MERGE INTO  `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_JOBS_PDW_DAG_T` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        ...
        TARGET.task_run_time = SOURCE.task_run_time,
