CREATE OR REPLACE VIEW `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_LINEAGE_COLLIBRA_V`
AS
SELECT
    ...
    'BigQuery Operations' AS SOURCE_COMMUNITY,
    ...
    'Pharma_Data_Warehouse_'||TRGT_TABLE_NAME AS MAPPING_SPECIFICATION_NAME,
    'Pharma_Data_Warehouse_'||TRGT_TABLE_NAME AS MAPPING_SPECIFICATION_FULL_NAME,
    ...
    CASE
     WHEN REGEXP_CONTAINS(TRGT_COLUMN_TECH_DESC, ';') THEN REGEXP_EXTRACT(TRGT_COLUMN_TECH_DESC, r';(.*)') --- Removing and text before the first semi colon
     ELSE 'Move as is.'
    END AS TRANSFORMATION_LOGIC,
    CASE
     WHEN TRGT_SCHEMA_NAME LIKE '%PR_SALES%' AND REGEXP_CONTAINS(TRGT_COLUMN_BUSINESS_DESC, ';') THEN REGEXP_EXTRACT(TRGT_COLUMN_BUSINESS_DESC, r'^(.*?);') -- Exception for Puerto Rico
