CREATE OR REPLACE PROCEDURE `PROJECT_ID`.D1_PHM_DW_LOG.SP_ADMIN_LINEAGE_TRUNCATE_LOAD()
/*
 This stored procedure `PROJECT_ID`.D1_PHM_DW_LOG.SP_ADMIN_LINEAGE_TRUNCATE_LOAD() retrieves the source-to-target mapping information from the stored procedure runs in the `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_JOBS_T` table. It populates the `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_LINEAGE_T` table with the mapping details at both the entity level and column level.
 
 The procedure performs the following steps:
 1. Extracts the source-to-target mapping at the entity level by joining the `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_JOBS_T` table with the referenced tables and applies ranking logic to determine the quality of the match.
 2. Filters the extracted mappings based on certain conditions and stores them in temporary tables.
 3. Combines the mappings from the entity level with the mappings from the column level, applying additional ranking logic to determine the quality of the match.
 4. Joins the mappings with other metadata tables (`PROJECT_ID.D1_PHM_DW_LOG.ADMIN_LOOKER_METADATA_COLUMNS_V`, `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_ENTITY_INVENTORY_T`, `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_DATASET_V`) to retrieve additional information about the target tables and columns.
 5. Joins the mappings with the `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_JOBS_PDW_AIRFLOW_T` table to retrieve information about the stored procedure runs in the Airflow environment.
 6. Filters the final result based on certain conditions and returns the result set.
 
 The stored procedure can be executed to update the `PROJECT_ID.D1_PHM_DW_LOG.ADMIN_LINEAGE_T` table with the latest source-to-target mapping information.
 
 */
BEGIN TRUNCATE TABLE `PROJECT_ID`.D1_PHM_DW_LOG.ADMIN_LINEAGE_T;

INSERT INTO
  `PROJECT_ID`.D1_PHM_DW_LOG.ADMIN_LINEAGE_T  --- Extract the Source to Target Mapping from Stored Proc Runs (Entity Level)
...
SELECT DISTINCT
  ...
  CASE
      WHEN SRC_TABLE_NAME = TRGT_TABLE_NAME THEN CAST(CONCAT(98., entities_union.MATCH_RANK) AS NUMERIC)
      WHEN columns_src.column_name = columns_target.column_name THEN 1
      WHEN CONCAT(REPLACE(columns_src.column_name,"_CV",""),'_',SRC_TABLE_NAME) LIKE  CONCAT('%',columns_target.column_name, '%') THEN CAST(CONCAT(2., entities_union.MATCH_RANK) AS NUMERIC)
      WHEN columns_target.column_name LIKE CONCAT(columns_src.column_name, '%') 
       AND  columns_target.column_name LIKE CONCAT('%',SRC_TABLE_NAME, '%') THEN CAST(CONCAT(3., entities_union.MATCH_RANK) AS NUMERIC)
      WHEN columns_target.column_name LIKE CONCAT(columns_src.column_name, '%') 
       AND REGEXP_CONTAINS(REPLACE(SRC_TABLE_NAME, '__', '_'), CONCAT('_', REGEXP_EXTRACT(columns_target.column_name, r'_(\w+)$'))) IS TRUE THEN CAST(CONCAT(4., entities_union.MATCH_RANK) AS NUMERIC)
      WHEN columns_target.column_name LIKE CONCAT(columns_src.column_name, '%') THEN CAST(CONCAT(5., entities_union.MATCH_RANK) AS NUMERIC)
    ELSE CAST(CONCAT(99., entities_union.MATCH_RANK) AS NUMERIC)
...
SELECT 
    ...
    CAST(logic.MATCH_RANK_ID AS INT64) AS MATCH_RANK_ID,
    logic.SORT_RANK_ID,
