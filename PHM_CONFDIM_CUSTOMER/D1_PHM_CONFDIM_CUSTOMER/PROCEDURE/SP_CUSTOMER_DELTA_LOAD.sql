CREATE PROCEDURE `PROJECT_ID`.D1_PHM_CONFDIM_CUSTOMER.SP_CUSTOMER_DELTA_LOAD()
BEGIN
...
SELECT
CONCAT(SRC.KUNNR_KNA1,SRC.START_DTE) AS COMPOSITE_KEY, SRC.D0_ACTION_ID, TGT.ROW_ADD_STP, TGT.ROW_ADD_USER_ID
FROM
PROJECT_ID.VI2_PHM_CONFDIM_CUSTOMER.PHM_ORP_CUSTOM__CUSTOMERMASTER_SCD_CV  SRC
LEFT JOIN `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.CUSTOMER_CV` TGT
ON CONCAT(SRC.KUNNR_KNA1,SRC.START_DTE) = TGT.COMPOSITE_KEY
...
DELETE FROM `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.CUSTOMER_CV` WHERE COMPOSITE_KEY IN (SELECT COMPOSITE_KEY FROM CUST_LATEST_RECORDS_TEMP WHERE D0_ACTION_ID = 'UPDATE') ;
...
ON CONCAT(CUST.KUNNR_KNA1,CUST.START_DTE) = TEMPD.COMPOSITE_KEY
