CREATE PROCEDURE `PROJECT_ID`.D1_PHM_CONFDIM_CUSTOMER.DO_NOT_MOVE_TEST_CUSTOMER_DELTA_LOAD()
BEGIN
...
SET v_stored_proc_name = 'D1_PHM_CONFDIM_CUSTOMER.SP_CUSTOMER_DELTA_LOAD';
SET v_proc_timestamp = (SELECT IFNULL(SP_CUSTOMER_DELTA_LOAD_STP, TIMESTAMP '1900-01-01 00:00:00') FROM `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.LAST_TIMESTAMP_CUSTOMER_EV`);
...
SELECT
CUST.*
,CONCAT(CUST.KUNNR_KNA1,CUST.START_DTE) AS PRIMARY_COMPOSITE_KEY
FROM
PROJECT_ID.VI2_PHM_CONFDIM_CUSTOMER.PHM_ORP_CUSTOM__CUSTOMERMASTER_SCD_CV  CUST
WHERE D0_CURR_VRSN_FLG = 'Y'
AND D0_START_STP>v_proc_timestamp
;

CREATE OR REPLACE TEMPORARY TABLE CUST_LATEST_RECORDS_TEMP_SCD_NU AS
SELECT
SRC.*,TGT.ROW_ADD_STP AS ROW1_ADD_STP
FROM PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.CUSTOMER_CV TGT
INNER JOIN CUST_LATEST_RECORDS_TEMP SRC
ON CONCAT(TGT.KUNNR_KNA1,TGT.VRSN_START_DTE)= SRC.PRIMARY_COMPOSITE_KEY;

DELETE FROM `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.CUSTOMER_CV` WHERE CONCAT(KUNNR_KNA1,VRSN_START_DTE) IN (SELECT PRIMARY_COMPOSITE_KEY FROM CUST_LATEST_RECORDS_TEMP_SCD_NU) ;
...
,CUST.KUNNR_KNA1 AS COMPOSITE_KEY
,CUST.ROW1_ADD_STP AS ROW_ADD_STP
,v_stored_proc_name AS ROW_ADD_USER_ID
,v_start_stp AS ROW_UPDATE_STP
,v_stored_proc_name AS ROW_UPDATE_USER_ID

FROM CUST_LATEST_RECORDS_TEMP_SCD_NU CUST

INNER JOIN KNA1_TEMP KNA1 ON
...
CREATE OR REPLACE TEMPORARY TABLE CUST_LATEST_RECORDS_TEMP_SCD_I AS
SELECT
CUST.*
FROM CUST_LATEST_RECORDS_TEMP CUST
WHERE
PRIMARY_COMPOSITE_KEY NOT IN (SELECT PRIMARY_COMPOSITE_KEY FROM CUST_LATEST_RECORDS_TEMP_SCD_NU);

UPDATE PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.CUSTOMER_CV SET VRSN_END_DTE=CURRENT_DATE()-1, CURR_VRSN_FLG ='N', ROW_UPDATE_STP=v_start_stp WHERE KUNNR_KNA1 IN (SELECT KUNNR_KNA1 FROM CUST_LATEST_RECORDS_TEMP_SCD_I WHERE END_DTE=CURRENT_DATE()-1 ) AND VRSN_END_DTE='9999-12-31';
...
FROM CUST_LATEST_RECORDS_TEMP_SCD_I CUST
