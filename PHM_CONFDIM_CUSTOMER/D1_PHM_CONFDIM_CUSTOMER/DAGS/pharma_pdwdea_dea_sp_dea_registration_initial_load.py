...
# DAG Definition
with DAG(
    'pharma_pdwdea_dea_sp_dea_registration_initial_load',
    default_args=default_args,
    description=f'Process for updating pharma_pdwdea_dea_sp_dea_registration_initial_load',
    schedule_interval=schedule_interval,
    start_date=pendulum.datetime(2023, 9, 8, tz='US/Eastern'),
    catchup=False,
    tags=['pdw', 'dea','init_load'],  # Added the dag_domain as a tag
    access_control= {"pdna" : ('can_read', 'can_edit')},
    concurrency=50,  # Add this line to set the concurrency
    max_active_runs = 1,
) as dag:

    execute_sp_dea_registration_initial_load = BigQueryInsertJobOperator(
        task_id='execute_sp_dea_registration_initial_load',
        configuration={
            'query': {
                'query': f"CALL `{project_id}`.D1_PHM_CONFDIM_CUSTOMER.SP_DEA_REGISTRATION_INITIAL_LOAD();",
