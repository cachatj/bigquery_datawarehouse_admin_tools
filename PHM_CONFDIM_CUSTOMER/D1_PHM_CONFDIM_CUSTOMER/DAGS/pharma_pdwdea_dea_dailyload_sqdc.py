...
if airflow_env == "dev":
    ...
    schedule_interval='030 3 * * *'


# DAG Definition
with DAG(
    'pharma_pdwdea_dea_dailyload_sqdc',
    default_args=default_args,
    description=f'Process for updating pharma_pdwdea_dea_dailyload_sqdc',
    schedule_interval=schedule_interval,
    start_date=pendulum.datetime(2023, 9, 8, tz='US/Eastern'),
    catchup=False,
    tags=['pdw', 'dea','seq_load'],  # Added the dag_domain as a tag
    access_control= {"pdna" : ('can_read', 'can_edit')},
    concurrency=50,  # Add this line to set the concurrency
    max_active_runs = 1,
) as dag:

    execute_sp_dea_registration_delta_load = BigQueryInsertJobOperator(
        task_id='execute_sp_dea_registration_delta_load',
        configuration={
            'query': {
                'query': f"CALL `{project_id}`.D1_PHM_CONFDIM_CUSTOMER.SP_DEA_REGISTRATION_DELTA_LOAD();",
    ...
    create_snow_alert = ServiceNowAlertingOperator(
        ...
        message=f"[PDW]pharma_pdwdea_dea_dailyload_sqdc has failed or has failed tasks",
        severity='3',
        source='airflow.cardinalhealth.net',
        additional_details=f"Additional details of failure available at http://airflow.cardinalhealth.net/dags/pharma_pdwdea_dea_dailyload_sqdc/grid",
    ...
    execute_sp_dea_registration_delta_load >> create_snow_alert