    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.SP_COMMITMENT_LEVEL_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for COMMITMENT_LEVEL_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_CUSTOMER.COMMITMENT_LEVEL_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.COMMITMENT_LEVEL_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.COMMITMENT_LEVEL_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.YYCOMMITMNT_YTSD_COMMIT_TXT = SOURCE.YYCOMMITMNT_YTSD_COMMIT_TXT,
        TARGET.YYCOMMITMNT_DES_YTSD_COMMIT_TXT = SOURCE.YYCOMMITMNT_DES_YTSD_COMMIT_TXT,
    ...
    WHEN NOT MATCHED THEN
      INSERT VALUES
      (
        SOURCE.YYCOMMITMNT_YTSD_COMMIT_TXT,
        SOURCE.YYCOMMITMNT_DES_YTSD_COMMIT_TXT,
        SOURCE.ROW_ADD_STP,
