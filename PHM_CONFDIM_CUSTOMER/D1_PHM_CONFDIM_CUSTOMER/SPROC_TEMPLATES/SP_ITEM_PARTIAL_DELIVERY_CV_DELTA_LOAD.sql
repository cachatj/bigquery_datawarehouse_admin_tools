    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.SP_ITEM_PARTIAL_DELIVERY_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for ITEM_PARTIAL_DELIVERY_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_CUSTOMER.ITEM_PARTIAL_DELIVERY_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.ITEM_PARTIAL_DELIVERY_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.ITEM_PARTIAL_DELIVERY_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        ...
        TARGET.ITEM_PRTL_DELIVERIES_ALW_DESC_TXT = SOURCE.ITEM_PRTL_DELIVERIES_ALW_DESC_TXT,
    ...
    WHEN NOT MATCHED THEN
      ...
      (
        SOURCE.KZTLF_KNVV,
        SOURCE.ITEM_PRTL_DELIVERIES_ALW_DESC_TXT,
        SOURCE.ROW_ADD_STP,
