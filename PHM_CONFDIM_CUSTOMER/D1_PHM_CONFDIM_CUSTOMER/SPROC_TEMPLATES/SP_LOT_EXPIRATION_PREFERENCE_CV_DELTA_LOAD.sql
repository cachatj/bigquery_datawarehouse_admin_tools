    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.SP_LOT_EXPIRATION_PREFERENCE_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for LOT_EXPIRATION_PREFERENCE_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_CUSTOMER.LOT_EXPIRATION_PREFERENCE_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.LOT_EXPIRATION_PREFERENCE_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.LOT_EXPIRATION_PREFERENCE_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.YYLOTEXP_YTSD_LOT_EXP_TXT = SOURCE.YYLOTEXP_YTSD_LOT_EXP_TXT,
        TARGET.YYDES_YTSD_LOT_EXP_TXT = SOURCE.YYDES_YTSD_LOT_EXP_TXT,
    ...
    WHEN NOT MATCHED THEN
      INSERT VALUES
      (
        SOURCE.YYLOTEXP_YTSD_LOT_EXP_TXT,
        SOURCE.YYDES_YTSD_LOT_EXP_TXT,
        SOURCE.ROW_ADD_STP,
