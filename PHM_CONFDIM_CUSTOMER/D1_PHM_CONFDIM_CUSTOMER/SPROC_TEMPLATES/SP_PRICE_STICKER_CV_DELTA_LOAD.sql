    ...
    CREATE PROCEDURE  `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.SP_PRICE_STICKER_CV_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for PRICE_STICKER_CV
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D1_PHM_CONFDIM_CUSTOMER.PRICE_STICKER_CV_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.PRICE_STICKER_CV`);


    MERGE INTO  `PROJECT_ID.D1_PHM_CONFDIM_CUSTOMER.PRICE_STICKER_CV` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        TARGET.YYPRCSTKR_YTSD_PRC_STKR_TX = SOURCE.YYPRCSTKR_YTSD_PRC_STKR_TX,
        TARGET.YYDES_YTSD_PRC_STKR_TX = SOURCE.YYDES_YTSD_PRC_STKR_TX,
    ...
    WHEN NOT MATCHED THEN
      INSERT VALUES
      (
        SOURCE.YYPRCSTKR_YTSD_PRC_STKR_TX,
        SOURCE.YYDES_YTSD_PRC_STKR_TX,
        SOURCE.ROW_ADD_STP,
