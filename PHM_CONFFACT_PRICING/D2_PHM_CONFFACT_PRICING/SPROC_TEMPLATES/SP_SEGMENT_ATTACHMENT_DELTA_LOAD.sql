    ...
    CREATE PROCEDURE  `PROJECT_ID.D2_PHM_CONFFACT_PRICING.SP_SEGMENT_ATTACHMENT_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for SEGMENT_ATTACHMENT
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D2_PHM_CONFFACT_PRICING.SEGMENT_ATTACHMENT_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D2_PHM_CONFFACT_PRICING.SEGMENT_ATTACHMENT`);


    MERGE INTO  `PROJECT_ID.D2_PHM_CONFFACT_PRICING.SEGMENT_ATTACHMENT` AS target
     USING 
      (
