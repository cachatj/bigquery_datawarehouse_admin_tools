    ...
    CREATE PROCEDURE  `PROJECT_ID.D2_PHM_CONFFACT_PRICING.SP_GRP_FUTURE_PRICE_TEMP_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for GRP_FUTURE_PRICE_TEMP
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D2_PHM_CONFFACT_PRICING.GRP_FUTURE_PRICE_TEMP_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D2_PHM_CONFFACT_PRICING.GRP_FUTURE_PRICE_TEMP`);


    MERGE INTO  `PROJECT_ID.D2_PHM_CONFFACT_PRICING.GRP_FUTURE_PRICE_TEMP` AS target
     USING 
      (
    ...
    WHEN MATCHED THEN
      UPDATE SET
        ...
        TARGET.cntrct_desc = SOURCE.cntrct_desc,
        ...
        TARGET.chg_cde = SOURCE.chg_cde,
        TARGET.change_effective_year = SOURCE.change_effective_year,
        TARGET.change_effective_month = SOURCE.change_effective_month,
        TARGET.change_effective_date = SOURCE.change_effective_date,
        TARGET.chg_time = SOURCE.chg_time,
        ...
        TARGET.item_effective_year = SOURCE.item_effective_year,
        TARGET.item_effective_month = SOURCE.item_effective_month,
    ...
    WHEN NOT MATCHED THEN
      ...
      (
        ...
        SOURCE.chg_cde,
        SOURCE.change_effective_year,
        SOURCE.change_effective_month,
        SOURCE.change_effective_date,
        SOURCE.chg_time,
        ...
        SOURCE.item_effective_year,
        SOURCE.item_effective_month,
