    ...
    CREATE PROCEDURE  `PROJECT_ID.D2_PHM_CONFFACT_PRICING.SP_ITEM_BLOCK_TEMP_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for ITEM_BLOCK_TEMP
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D2_PHM_CONFFACT_PRICING.ITEM_BLOCK_TEMP_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D2_PHM_CONFFACT_PRICING.ITEM_BLOCK_TEMP`);


    MERGE INTO  `PROJECT_ID.D2_PHM_CONFFACT_PRICING.ITEM_BLOCK_TEMP` AS target
     USING 
      (
