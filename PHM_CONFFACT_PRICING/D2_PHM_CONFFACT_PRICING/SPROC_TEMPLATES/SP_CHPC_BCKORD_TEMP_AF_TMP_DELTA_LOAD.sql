    ...
    CREATE PROCEDURE  `PROJECT_ID.D2_PHM_CONFFACT_PRICING.SP_CHPC_BCKORD_TEMP_AF_TMP_DELTA_LOAD`()
    BEGIN
    /***********************************************************************************************************************

    Author: <Automatically Written Using Code Generator>
    Creation Date: <Insert Date>
    Sproc Purpose: Delta Loading Merge Sproc for CHPC_BCKORD_TEMP_AF_TMP
    ...
    ************************************************************************************************************************/
    ...
    SET v_stored_proc_name = 'D2_PHM_CONFFACT_PRICING.CHPC_BCKORD_TEMP_AF_TMP_DELTA_LOAD';
    SET last_load_timestamp = (SELECT MAX(ROW_ADD_STP) FROM `PROJECT_ID.D2_PHM_CONFFACT_PRICING.CHPC_BCKORD_TEMP_AF_TMP`);


    MERGE INTO  `PROJECT_ID.D2_PHM_CONFFACT_PRICING.CHPC_BCKORD_TEMP_AF_TMP` AS target
     USING 
      (
